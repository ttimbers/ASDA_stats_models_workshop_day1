...
```{r plot fruitfly_2_groups data, fig.height = 4, fig.width = 4}
...
```{r plot fruitfly_3_groups data, fig.height = 4, fig.width = 5}
...
```{r pairwise comparisons of fruitfly_3_groups}
...
```{r Tukeys HSD of fruitfly_3_groups}
...
- $H_{0} : \mu_{short} = \mu_{long}$
- $H_{A}$ : $\mu_{short} \ne \mu_{long}$

Now that we have our case setup, let's load the data and explore it:

```{r load and look at fruitfly_thorax_len data}
# load data
...
```{r plot 2-way ANOVA with 2 groups, fig.height = 4, fig.width = 5}
# re-order factors to make them show up how we would like them on the plot
# instead of alphabetically (default R behaviour)
...
Now we see that we get back an additional line in our results summary that corresponds to the hypotheses regarding the effect of body size/thoraz length on longevity. The p-value from this line, ~ 1.55 x $10^{-08}$, is very very small, and thus we can reject the null hypothesis ($\mu_{short} = \mu_{long}$) and infer that body size/thorax length has a statistically signifcant effect on longevity.

We also see, that in this case, we again observe a very small p-value, ~ 4.84 x $10^{-11}$, for the factor/category/explanatory variable of sexual activity, and thus we also reject the null hypothesis ($\mu_{No} = \mu_{Yes}$) and infer that sexual activity also has a statistically signifcant effect on longevity.
...
- $H_{0}$ : There is **no** interaction effect between sexual activity and thorax length on the mean longevity of the population.
- $H_{A}$ : There is an interaction effect between sexual activity and thorax length on the mean longevity of the population.
...
```{r plot ANOVA with interaction, fig.height = 4, fig.width = 5}
...
```{r 2-way ANOVA with 2 groups incl interaction term}
# create an ANOVA "model" object
fruitfly_thorax_len_model <- aov(longevity ~ sexually_active * thorax,
                               data = fruitfly_thorax_len)
